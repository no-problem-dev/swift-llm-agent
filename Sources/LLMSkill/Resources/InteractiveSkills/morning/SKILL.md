...
### Step 2: 統合ブリーフィングの提示
取得データを統合し、以下の構造でブリーフィングを作成して `request_user_input` の description に含めて提示する:
...
### Step 3: 追加アクション
- **予定を追加** → `request_user_input` で予定の内容を聞き → `pick_date` → `add_calendar_event` でカレンダーに登録
- **優先事項を整理** → `request_user_input` で今日やりたいことを聞き、時間枠と優先度を踏まえて整理して提示
- **詳しく聞く** → `request_user_input`（type: "selection"）で詳しく知りたい項目（天気の詳細/予定の詳細/健康の詳細）を選択
