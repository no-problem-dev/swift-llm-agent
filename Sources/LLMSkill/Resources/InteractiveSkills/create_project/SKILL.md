...
### Step 1: プロジェクト名の収集
`request_user_input` でプロジェクト名を聞く:
- question: "新しいプロジェクトの名前を入力してください。\n\n例: 「家計管理アプリ」「ブログ記事執筆」「英語学習」"

### Step 2: 指示の追加確認
`request_user_input`（type: "selection"）で指示の追加有無を確認する:
- question: "プロジェクト「{name}」に、AIアシスタントへの指示を追加しますか？\n\n指示を追加すると、このプロジェクトのセッションでAIが自動的にその指示に従います。"
- 選択肢: 「指示を追加する」「指示なしで作成」

### Step 3: 指示テキストの収集（指示ありの場合）
「指示を追加する」が選択された場合、`request_user_input` で指示テキストを収集する:
- question: "プロジェクトの指示を入力してください。\n\nAIアシスタントがこのプロジェクトのセッションで常に従う指示です。\n\n例:\n- 「常にTypeScriptで回答してください」\n- 「文章は敬体（です・ます調）で書いてください」\n- 「コードレビュー時はセキュリティ観点を重視してください」"

### Step 4: 確認
`request_user_input`（type: "confirmation"）で全設定のプレビューを表示して確認する:
- question に以下を含める:
  - プロジェクト名
  - 指示内容（ある場合）
- message: "この内容でプロジェクトを作成しますか？"
